...
samples = 50  # Number of samples in the dataset
...
cv_mse = -np.mean(score)  # Best position for the interrogator (seismometer) based on the coefficients

regressor.fit(X, y)  # fit model on full dataset to analyze coefficients

nb_features_per_interrogator = X.shape[1] // 2  # Number of features per interrogator

# Compute mean absolute coefficient for each interrogator
coef1 = np.mean(np.abs(regressor.coef_[:, :nb_features_per_interrogator]))  # Coefficients for Interrogator 1
coef2 = np.mean(np.abs(regressor.coef_[:, nb_features_per_interrogator:]))  # Coefficients for Interrogator 2

print(f"Average coefficient Interrogator : {interrogators[0]}: {coef1:.3f}")
print(f"Average coefficient Interrogator : {interrogators[1]}: {coef2:.3f}")

# Select the best interrogator
#best_interrogator = interrogators[0] if coef1 > coef2 else interrogators[1]
print(f"Best position for the seismometer based on the coefficients : {best_interrogator}")
...
samples = 50  # Number of samples in the dataset
dataset = VectorAcousticWaveDataset3D(samples, interrogators=[(10, 0, 0), (-10, 0, 0)])
...
for idx in range(samples):
    y.append(dataset.get_epicenter(idx))
    #y.append(dataset.get_epicenter(idx))  # add a second tile to match the sample size of X
    experiment = dataset[idx]  # Get the experiment data
    X.append(experiment[1][(10, 0, 0)].T)  # Get the interrogator data of the first seismometer
    X.append(experiment[1][(-10, 0, 0)].T)  # Get the interrogator data of the second seismometer
X = np.array(X)  # Convert to NumPy
y = np.array(y)
print("y shape :",y.shape)
print("X shape :",X.shape)
...
# def plot_NMSE_error_analysis(errors, models=None):
#     """ Plot the prediction error per experiment index as a bar chart """
#     # Plot error per experiment index as a bar chart
#     plt.figure(figsize=(8, 6))
#     plt.bar(range(len(errors)), errors, color='r', alpha=0.7, label="Absolute Error")
